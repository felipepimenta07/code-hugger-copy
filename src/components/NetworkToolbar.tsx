    ...
    <div className="absolute top-0 left-0 right-0 z-40 flex items-center justify-between px-3 py-1.5 bg-[hsl(220,20%,6%)]/95 backdrop-blur-md border-b border-border/20">
      ...
      <div className="flex items-center gap-1">
        <button onClick={onMasterView}
          ...
          <span className="flex items-center gap-1"><Layers size={12} /> Master</span>
        </button>
        <button onClick={onSingleView}
          ...
          <span className="flex items-center gap-1"><Target size={12} /> Single</span>
        </button>

        <div className="h-4 w-px bg-border/30 mx-1" />

        <button onClick={onFitToScreen}
          className="px-2 py-1 text-xs font-mono text-muted-foreground hover:text-foreground hover:bg-secondary/40 rounded transition-colors"
      ...
      <div className="flex items-center gap-2">
        <div className="relative w-44">
          ...
          <Input type="text" placeholder="Buscar nó..." value={searchQuery}
            ...
            className="pl-7 h-7 text-sm bg-secondary/30 border-border/20 rounded focus:border-primary/50" />
        </div>
            ...
            {viewMode === 'single' && onCreateNode && (
              <>
                <DropdownMenuItem onClick={onCreateNode}>
                  <Plus size={14} className="mr-2" /> Criar Nó
                </DropdownMenuItem>
                <DropdownMenuSeparator />
              </>
            )}
            <DropdownMenuItem onClick={onNewFlow}>
              <Plus size={14} className="mr-2" /> Novo Flow
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={onOpenFlows}>
              <Layers size={14} className="mr-2" /> Gerenciar Flows
            </DropdownMenuItem>
            {onOpenWhatsApp && (
              <DropdownMenuItem onClick={onOpenWhatsApp}>
                <MessageCircle size={14} className="mr-2 text-green-500" /> WhatsApp
              </DropdownMenuItem>
            )}
            {onLogout && (
              <>
